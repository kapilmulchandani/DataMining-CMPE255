...
def euclideanSimilarity():
    ...
    euclidSimilarity = (1 / 1 + euclideanDistance) * 100
...
def sineSimilarity():
    ...
    sumOfSquares = 0
    for i in range(len(queryTF)):
        crossProduct += queryTF[i] * databaseTF[i]
        sumOfSquares += (queryTF[i] - databaseTF[i]) ** 2
    ...
    matchPercentage = (float)(crossProduct / (queryVectorMagnitude * databaseVectorMagnitude)) * 100
